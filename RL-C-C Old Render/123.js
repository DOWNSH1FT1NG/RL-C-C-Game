...
class Chunk {
    // ... (весь класс Chunk остается без изменений, включая его конструктор)
    // ... (внутренняя логика спавна бонусов, такая как `if (Math.random() < settings.POWERUP_SPAWN_CHANCE)`, остается здесь)
    // ... (это важно, так как чанк сам решает, есть ли на нем бонус, а менеджер его только активирует)
    constructor(scene, chunkX, chunkZ, playerChunkX, playerChunkZ) {
        this.scene = scene;
        this.chunkX = chunkX;
        this.chunkZ = chunkZ;

        const geometry = new THREE.PlaneGeometry(settings.CHUNK_SIZE, settings.CHUNK_SIZE, settings.CHUNK_SEGMENTS, settings.CHUNK_SEGMENTS);
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            if (cyclicDist < GRASSLAND_END) {
                ...
                biomeColor.lerpColors(colorGrass, colorDesert, t);
            } else if (cyclicDist < DESERT_END) {
                biomeHeight = hDesert;
                biomeColor.copy(colorDesert);
                ...
                biomeColor.lerpColors(colorMountain, colorGrass, t);
            }
            let finalHeight = biomeHeight;
            let finalColor = biomeColor.clone();
            const roadColor = new THREE.Color(0x808080);
            const roadData = getRoadData(worldX, worldZ);
            if (roadData.onPavement) {
                finalHeight = roadData.roadHeight;
                finalColor.copy(roadColor);
            } else if (roadData.onShoulder) {
                finalHeight = lerp(biomeHeight, roadData.roadHeight, roadData.shoulderBlendFactor);
                finalColor.lerpColors(biomeColor, roadColor, roadData.shoulderBlendFactor);
            }
            const checkerboardSize = 25;
            const cx = Math.floor(worldX / checkerboardSize);
            const cz = Math.floor(worldZ / checkerboardSize);
            const isEvenSquare = (cx + cz) % 2 === 0;
            if (!roadData.onPavement && !isEvenSquare) {
                finalColor.multiplyScalar(0.9);
            }
            if (finalHeight > settings.MOUNTAIN_SNOW_HEIGHT) {
                const snowT = Math.min((finalHeight - settings.MOUNTAIN_SNOW_HEIGHT) / (settings.BIOME_BLEND_RANGE * 5), 1.0);
                finalColor.lerp(colorSnow, snowT);
            }
        ...
        let sumY = 0; let sumR = 0; let sumG = 0; let sumB = 0;
        const posAttr = geometry.attributes.position;
        const colAttr = geometry.attributes.color;
        for (let vi = 0; vi < posAttr.count; vi++) {
            sumY += posAttr.getY(vi);
            if (colAttr) { sumR += colAttr.getX(vi); sumG += colAttr.getY(vi); sumB += colAttr.getZ(vi); }
        }
        const avgY = sumY / posAttr.count;
        let avgColor = null;
        if (colAttr) { avgColor = new THREE.Color(sumR / posAttr.count, sumG / posAttr.count, sumB / posAttr.count); }
        const centerWorldX = chunkX * settings.CHUNK_SIZE;
        const centerWorldZ = chunkZ * settings.CHUNK_SIZE;
        const centerRoad = getRoadData(centerWorldX, centerWorldZ);
        if (centerRoad && centerRoad.onPavement) { this.mesh.userData.surfaceType = 'road'; }
        else if (avgY >= (settings.MOUNTAIN_SNOW_HEIGHT - 8)) { this.mesh.userData.surfaceType = 'snow'; }
        else if (avgColor) {
            const grassCol = new THREE.Color(0x51c74f);
            const desertCol = new THREE.Color(0xf0e68c);
            const mountainCol = new THREE.Color(settings.BIOMES && settings.BIOMES[2] ? settings.BIOMES[2].color : 0x888888);
            const colorDiff = (a, b) => Math.abs(a.r - b.r) + Math.abs(a.g - b.g) + Math.abs(a.b - b.b);
            const dg = colorDiff(avgColor, grassCol);
            const dd = colorDiff(avgColor, desertCol);
            const dm = colorDiff(avgColor, mountainCol);
            const min = Math.min(dg, dd, dm);
            if (min === dg) this.mesh.userData.surfaceType = 'grass';
            else if (min === dd) this.mesh.userData.surfaceType = 'desert';
            else this.mesh.userData.surfaceType = 'mountain';
        } else { this.mesh.userData.surfaceType = 'default'; }
        this.mesh.castShadow = true;
        this.mesh.receiveShadow = true;
        this.mesh.position.set(chunkX * settings.CHUNK_SIZE, 0, chunkZ * settings.CHUNK_SIZE);
        this.scene.add(this.mesh);

        if (Math.random() < settings.OBJECT_SPAWN_CHANCE) {
            if (coinPatternManager.shouldSpawnPattern()) {
                this.spawnCoinPattern();
            } else {
                this.spawnSingleCoin();
            }
        }
        if (Math.random() < settings.POWERUP_SPAWN_CHANCE) {
            const powerupSprite = new THREE.Sprite(powerupMaterial);
            powerupSprite.scale.set(settings.POWERUP_SCALE, settings.POWERUP_SCALE, 1);
            const shadow = new THREE.Mesh(shadowGeometry, shadowMaterial);
            shadow.rotation.x = -Math.PI / 2;
            shadow.scale.set(0.7, 0.7, 0.7);
            const localX = (Math.random() - 0.5) * settings.CHUNK_SIZE;
            const localZ = (Math.random() - 0.5) * settings.CHUNK_SIZE;
            const raycaster = new THREE.Raycaster();
            this.mesh.updateMatrixWorld();
            const rayOrigin = new THREE.Vector3(localX + this.mesh.position.x, 1000, localZ + this.mesh.position.z);
            raycaster.set(rayOrigin, new THREE.Vector3(0, -1, 0));
            const intersects = raycaster.intersectObject(this.mesh, false);
            let groundY = this.mesh.position.y;
            if (intersects.length > 0) { groundY = intersects[0].point.y; }
            this.initialPowerupY = groundY + settings.POWERUP_SCALE / 2 + settings.COIN_HOVER_HEIGHT * 2;
            powerupSprite.position.set(localX, this.initialPowerupY - this.mesh.position.y, localZ);
            shadow.position.set(localX, groundY - this.mesh.position.y + 0.1, localZ);
            this.mesh.add(powerupSprite);
            this.mesh.add(shadow);
            this.powerup = powerupSprite;
            this.powerupShadow = shadow;
        }
        if (Math.random() < settings.SPEEDUP_SPAWN_CHANCE) {
            const speedupSprite = new THREE.Sprite(speedupMaterial);
            speedupSprite.scale.set(settings.SPEEDUP_SCALE, settings.SPEEDUP_SCALE, 1);
            const shadow = new THREE.Mesh(shadowGeometry, shadowMaterial);
            shadow.rotation.x = -Math.PI / 2;
            shadow.scale.set(0.7, 0.7, 0.7);
            const localX = (Math.random() - 0.5) * settings.CHUNK_SIZE;
            ...
            let groundY = this.mesh.position.y;
            if (intersects.length > 0) { groundY = intersects[0].point.y; }
            this.initialSpeedupY = groundY + settings.SPEEDUP_SCALE / 2 + settings.COIN_HOVER_HEIGHT * 2;
            speedupSprite.position.set(localX, this.initialSpeedupY - this.mesh.position.y, localZ);
            shadow.position.set(localX, groundY - this.mesh.position.y + 0.1, localZ);
            this.mesh.add(speedupSprite);
            this.mesh.add(shadow);
            this.speedup = speedupSprite;
            this.speedupShadow = shadow;
        }
        if (Math.random() < settings.SUNRISE_SPAWN_CHANCE) {
            const sunriseSprite = new THREE.Sprite(sunriseMaterial);
            sunriseSprite.scale.set(settings.SUNRISE_SCALE, settings.SUNRISE_SCALE, 1);
            const shadow = new THREE.Mesh(shadowGeometry, shadowMaterial);
            shadow.rotation.x = -Math.PI / 2;
            shadow.scale.set(0.7, 0.7, 0.7);
            const localX = (Math.random() - 0.5) * settings.CHUNK_SIZE;
            ...
            let groundY = this.mesh.position.y;
            if (intersects.length > 0) { groundY = intersects[0].point.y; }
            this.initialSunriseY = groundY + settings.SUNRISE_SCALE / 2 + settings.COIN_HOVER_HEIGHT * 2;
            sunriseSprite.position.set(localX, this.initialSunriseY - this.mesh.position.y, localZ);
            shadow.position.set(localX, groundY - this.mesh.position.y + 0.1, localZ);
            this.mesh.add(sunriseSprite);
            this.mesh.add(shadow);
            this.sunrise = sunriseSprite;
            this.sunriseShadow = shadow;
        }
    }
    createCoinAt(localX, localZ) {
        const raycaster = new THREE.Raycaster();
        this.mesh.updateMatrixWorld();
        const rayOrigin = new THREE.Vector3(localX + this.mesh.position.x, 1000, localZ + this.mesh.position.z);
        raycaster.set(rayOrigin, new THREE.Vector3(0, -1, 0));
        ...
        if (intersects.length > 0) {
            ...
            const coin = new THREE.Mesh(objectGeometry, isXRayEffectActive ? highlightMaterial : objectMaterial);
            coin.castShadow = false;
            coin.receiveShadow = false;
            coin.rotation.x = Math.PI / 2;
            coin.scale.set(settings.COIN_SCALE, settings.COIN_SCALE, settings.COIN_SCALE);
            const initialCoinY = groundY + coinRadius + settings.COIN_HOVER_HEIGHT;
    ...
    spawnCoinPattern() {
        const pattern = coinPatternManager.getRandomPattern();
        const margin = 20;
        const centerX = (Math.random() - 0.5) * (settings.CHUNK_SIZE - margin * 2);
        const centerZ = (Math.random() - 0.5) * (settings.CHUNK_SIZE - margin * 2);
        console.log(`Спавним паттерн "${pattern.name}" в чанке [${this.chunkX}, ${this.chunkZ}]`);
    ...
    collectSunrise() {
        if (this.sunrise) { this.mesh.remove(this.sunrise); this.sunrise = null; }
        if (this.sunriseShadow) { this.mesh.remove(this.sunriseShadow); this.sunriseShadow = null; }
    }
    update(delta) {
        for (let i = 0; i < this.coins.length; i++) {
            ...
            coin.position.y = hoverY - this.mesh.position.y;
        }
        if (this.powerup) {
            const hoverY = this.initialPowerupY + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE;
            this.powerup.position.y = hoverY - this.mesh.position.y;
        }
        if (this.speedup) {
            const hoverY = this.initialSpeedupY + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE;
            this.speedup.position.y = hoverY - this.mesh.position.y;
        }
        if (this.sunrise) {
            const hoverY = this.initialSunriseY + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE;
            this.sunrise.position.y = hoverY - this.mesh.position.y;
        }
    }
    destroy() {
