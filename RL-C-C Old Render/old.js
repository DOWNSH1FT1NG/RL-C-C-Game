...
function createAlea(seed) {
    // Преобразуем числовой сид в строку для более надежного хеширования
    const seedStr = String(seed);

    // Внутренняя функция для "перемешивания" строки в число (хеширование)
    function mash(data) {
        let n = 0xefc8249d;
    ...
    // Инициализируем внутреннее состояние генератора на основе сида
    let s0 = mash(' ');
    let s1 = mash(' ');
    let s2 = mash(' ');
    let c = 1;
    ...
    // Возвращаем саму функцию, которая будет генерировать числа
    return function () {
...
const MIN_Y_LEVEL = -200; // Уровень, ниже которого игрок телепортируется наверх
const LOD_LEVELS = [
    { distance: 8, segments: 16 },
    { distance: 14, segments: 8 }, 
...
const hoverRaycaster = new THREE.Raycaster(); // <-- ДОБАВЬТЕ ЭТУ СТРОКУ

const chunksToLoad = []; // Очередь чанков, ожидающих создания
const CHUNKS_PER_FRAME = 32; // Сколько чанков мы создаем за один кадр. Можете настроить это значение.
...
const prng = createAlea(WORLD_SEED); // 1. Создаем сам генератор на основе сида
const terrainNoise = createNoise2D(prng); // 2. Передаем этот генератор в createNoise2D
...
function getRoadData(worldX, worldZ) {
    // --- НАСТРОЙКИ ФОРМЫ ДОРОГИ ---
    // Увеличьте, чтобы повороты были более плавными и редкими. Уменьшите для "серпантина".
    const ROAD_SHAPE_SCALE = 0.00019;

    // --- НАСТРОЙКИ ВЫСОТЫ ДОРОГИ ---
    const ROAD_HEIGHT_SCALE = 0.0008; // Частота холмов на дороге
    const ROAD_HEIGHT_AMPLITUDE = 40;   // Высота холмов на дороге
    const ROAD_BASE_HEIGHT = 15;        // Базовая высота дороги

    // --- ШИРИНА ДОРОГИ И ОБОЧИНЫ ---
    // Теперь эти значения - это реальная ширина в мировых юнитах (метрах).
    const pavementWidth = 135; // Ширина асфальта (например, 24 метра)
    const shoulderWidth = 130; // Ширина обочины с каждой стороны

    // 1. ВЫЧИСЛЯЕМ ЗНАЧЕНИЕ ШУМА В ЦЕНТРЕ И У СОСЕДНИХ ТОЧЕК
    // Это нужно для вычисления градиента (крутизны)
    const epsilon = 1.0; // Маленькое расстояние для вычисления производной
    ...
    // 2. ВЫЧИСЛЯЕМ ГРАДИЕНТ И ЕГО ВЕЛИЧИНУ
    const dx = (nx1 - nx2) / (2 * epsilon); // Скорость изменения шума по оси X
    const dz = (nz1 - nz2) / (2 * epsilon); // Скорость изменения шума по оси Z
    const gradientMagnitude = Math.sqrt(dx * dx + dz * dz);

    // 3. ВЫЧИСЛЯЕМ СКОРРЕКТИРОВАННОЕ РАССТОЯНИЕ ДО ЦЕНТРА ДОРОГИ
    let distanceToCenter;
    // Если градиент очень мал (мы на "холме" или во "впадине" шума), дороги здесь нет
    if (gradientMagnitude < 1e-9) {
        distanceToCenter = Infinity;
    } else {
        // КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: делим значение шума на его крутизну.
        // Это дает нам реальное геометрическое расстояние до линии, где шум равен нулю.
        distanceToCenter = Math.abs(roadValue) / gradientMagnitude;
    }

    // 4. ПРОВЕРЯЕМ, НАХОДИМСЯ ЛИ МЫ НА ДОРОГЕ ИЛИ ОБОЧИНЕ
    const onPavement = distanceToCenter <= pavementWidth / 2;
    // Общая ширина: половина асфальта + вся обочина
    ...
    // 5. ВЫЧИСЛЯЕМ ВЫСОТУ ДОРОГИ (как и раньше)
    ...
    // 6. РАССЧИТЫВАЕМ СМЕШИВАНИЕ ДЛЯ ОБОЧИНЫ
    let shoulderBlendFactor = 1.0;
    if (onShoulder) {
        let blend = (totalHalfWidth - distanceToCenter) / shoulderWidth;
        shoulderBlendFactor = Math.max(0, Math.min(1, blend)) ** 2; // Квадрат для более резкого перехода
    }
...
class Chunk {
    constructor(scene, chunkX, chunkZ, distanceFromPlayer = 0) {
        this.scene = scene;
        this.chunkX = chunkX;
        this.chunkZ = chunkZ;
        this.mesh = null;
        ...
        // --- ИЗМЕНЕНИЕ: init теперь не вызывается напрямую ---
        this.init();
    }

    // --- НОВЫЙ МЕТОД: для перестройки геометрии ---
    rebuildMesh(segments) {
        // Если меш уже существует, удаляем его
        if (this.mesh) {
            this.scene.remove(this.mesh);
            this.mesh.geometry.dispose();
            // ВАЖНО: не удаляйте материал, он может использоваться другими чанками
        }

        this.currentLODSegments = segments;

        const geometry = new THREE.PlaneGeometry(settings.CHUNK_SIZE, settings.CHUNK_SIZE, segments, segments);
        geometry.rotateX(-Math.PI / 2);

        // --- Копируем сюда всю логику генерации высот и цветов из старого init() ---
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            let biomeHeight = 0;
            let biomeColor = new THREE.Color();
            if (cyclicDist < GRASSLAND_END) { biomeHeight = hGrass; biomeColor.copy(colorGrass); }
            else if (cyclicDist < GRASS_TO_DESERT_END) { const t = (cyclicDist - GRASSLAND_END) / settings.BIOME_BLEND_RANGE; biomeHeight = lerp(hGrass, hDesert, t); biomeColor.lerpColors(colorGrass, colorDesert, t); }
            else if (cyclicDist < DESERT_END) { biomeHeight = hDesert; biomeColor.copy(colorDesert); }
            else if (cyclicDist < DESERT_TO_MOUNTAIN_END) { const t = (cyclicDist - DESERT_END) / settings.BIOME_BLEND_RANGE; biomeHeight = lerp(hDesert, hMountain + settings.MOUNTAIN_BASE_HEIGHT, t); biomeColor.lerpColors(colorDesert, colorMountain, t); }
            else if (cyclicDist < MOUNTAIN_END) { biomeHeight = hMountain + settings.MOUNTAIN_BASE_HEIGHT; biomeColor.copy(colorMountain); }
            else { const t = (cyclicDist - MOUNTAIN_END) / settings.BIOME_BLEND_RANGE; biomeHeight = lerp(hMountain + settings.MOUNTAIN_BASE_HEIGHT, hGrass, t); biomeColor.lerpColors(colorMountain, colorGrass, t); }
            let finalHeight = biomeHeight;
            let finalColor = biomeColor.clone();
            const roadData = getRoadData(worldX, worldZ);
            if (roadData.onPavement) { finalHeight = roadData.roadHeight; finalColor.set(0x808080); }
            else if (roadData.onShoulder) { finalHeight = lerp(biomeHeight, roadData.roadHeight, roadData.shoulderBlendFactor); finalColor.lerpColors(biomeColor, new THREE.Color(0x808080), roadData.shoulderBlendFactor); }
            const cx = Math.floor(worldX / 25), cz = Math.floor(worldZ / 25);
            if (!roadData.onPavement && (cx + cz) % 2 !== 0) { finalColor.multiplyScalar(0.9); }
            if (finalHeight > settings.MOUNTAIN_SNOW_HEIGHT) { const snowT = Math.min((finalHeight - settings.MOUNTAIN_SNOW_HEIGHT) / (settings.BIOME_BLEND_RANGE * 5), 1.0); finalColor.lerp(colorSnow, snowT); }
        ...
        // --- ВОЗВРАЩАЕМ ВСЕ ОБЪЕКТЫ НА МЕСТО ---
        // Это нужно, чтобы монеты и бонусы не пропали при смене LOD
        this.coins.forEach(c => this.mesh.add(c));
        this.shadows.forEach(s => this.mesh.add(s));
        if (this.powerup) this.mesh.add(this.powerup);
        if (this.powerupShadow) this.mesh.add(this.powerupShadow);
        if (this.speedup) this.mesh.add(this.speedup);
        if (this.speedupShadow) this.mesh.add(this.speedupShadow);
        if (this.sunrise) this.mesh.add(this.sunrise);
        if (this.sunriseShadow) this.mesh.add(this.sunriseShadow);
        if (this.xray) this.mesh.add(this.xray);
        if (this.xrayShadow) this.mesh.add(this.xrayShadow);
    }

    // --- ИЗМЕНЕННЫЙ init() ---
    init() {
        // --- ЭТОТ БЛОК БЫЛ ПРОПУЩЕН ---
        // 1. Определяем нужный LOD на основе расстояния до игрока
        let segments = LOD_LEVELS[LOD_LEVELS.length - 1].segments;
        for (const level of LOD_LEVELS) {
            if (this.distanceFromPlayer <= level.distance) {
        ...
        // 2. Создаем меш с этим LOD (теперь `segments` определена)
        this.rebuildMesh(segments);

        // 3. Логика спавна объектов (монеты, паттерны)
        const chunkIdStr = `(${this.chunkX},${this.chunkZ})`;
        ...
        // 4. Логика спавна бонусов (которую мы вернули)
        if (bonusManager) {
            ...
            for (const bonus of spawnableBonuses) {
                if (bonusManager.shouldSpawnBonus(bonus.type, bonus.chance, this.chunkX, this.chunkZ)) {
                    this[bonus.spawnFunc](); // Вызываем метод спавна на ЭТОМ чанке (this)
                    break;
                }
            }
        }
    ...
    _spawnBonus(type, material, scale, storageKey) {
        ...
        switch (type) {
            case 'powerup': seedModX = 11; seedModZ = 12; break;
            case 'speedup': seedModX = 21; seedModZ = 22; break;
            case 'sunrise': seedModX = 31; seedModZ = 32; break;
            case 'xray': seedModX = 41; seedModZ = 42; break;
        }

        const localX = (seededRandom(this.chunkX * seedModX, this.chunkZ * seedModX, WORLD_SEED) - 0.5) * settings.CHUNK_SIZE;
        const localZ = (seededRandom(this.chunkX * seedModZ, this.chunkZ * seedModZ, WORLD_SEED) - 0.5) * settings.CHUNK_SIZE;
        ...
        this['initial' + storageKey.charAt(0).toUpperCase() + storageKey.slice(1) + 'Y'] = initialY;
    }
    ...
    spawnSingleCoin() {
        const localX = (seededRandom(this.chunkX + 5, this.chunkZ + 5, WORLD_SEED) - 0.5) * settings.CHUNK_SIZE;
        const localZ = (seededRandom(this.chunkX - 5, this.chunkZ - 5, WORLD_SEED) - 0.5) * settings.CHUNK_SIZE;
        this.createCoinAt(localX, localZ);
    }

    spawnCoinPattern() {
        const centerX = 0;
        const centerZ = 0;
        const worldX = this.chunkX * settings.CHUNK_SIZE + centerX;
        const worldZ = this.chunkZ * settings.CHUNK_SIZE + centerZ;
        ...
        for (const point of pattern.shape) {
            const localX = centerX + point.x;
            const localZ = centerZ + point.z;
            const yOffset = point.y || 0;
            this.createCoinAt(localX, localZ, yOffset);
        }
    }
    ...
    createCoinAt(localX, localZ, yOffset = 0) {
        ...
        const worldZ = this.chunkZ * settings.CHUNK_SIZE + localZ;
        const groundY = getTerrainHeightAt(worldX, worldZ);
        const coinRadius = 1.5 * settings.COIN_SCALE;
        const initialCoinY = groundY + coinRadius + settings.COIN_HOVER_HEIGHT + yOffset;
    ...
    collectPowerup() { if (this.powerup) { this.mesh.remove(this.powerup); this.powerup = null; } if (this.powerupShadow) { this.mesh.remove(this.powerupShadow); this.powerupShadow = null; } }
    collectSpeedup() { if (this.speedup) { this.mesh.remove(this.speedup); this.speedup = null; } if (this.speedupShadow) { this.mesh.remove(this.speedupShadow); this.speedupShadow = null; } }
    collectSunrise() { if (this.sunrise) { this.mesh.remove(this.sunrise); this.sunrise = null; } if (this.sunriseShadow) { this.mesh.remove(this.sunriseShadow); this.sunriseShadow = null; } }
    collectXRay() { if (this.xray) { this.mesh.remove(this.xray); this.xray = null; } if (this.xrayShadow) { this.mesh.remove(this.xrayShadow); this.xrayShadow = null; } }
    ...
    update(delta) {
        for (let i = 0; i < this.coins.length; i++) {
            ...
            const hoverY = data.initialY + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED + data.hoverOffset) * settings.COIN_HOVER_RANGE;
            coin.position.y = hoverY;
        }

        if (this.powerup) { this.powerup.position.y = this.initialPowerupY - this.mesh.position.y + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE; }
        if (this.speedup) { this.speedup.position.y = this.initialSpeedupY - this.mesh.position.y + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE; }
        if (this.sunrise) { this.sunrise.position.y = this.initialSunriseY - this.mesh.position.y + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE; }
        if (this.xray) { this.xray.position.y = this.initialXrayY - this.mesh.position.y + Math.sin(clock.getElapsedTime() * settings.COIN_HOVER_SPEED) * settings.COIN_HOVER_RANGE; }
    }

    destroy() {
        const worldX = this.chunkX * settings.CHUNK_SIZE;
        const worldZ = this.chunkZ * settings.CHUNK_SIZE;
        coinPatternManager.unregisterPatternSpawn(worldX, worldZ);
        ...
        for (const coin of this.coins) { coin.geometry.dispose(); }
        for (const shadow of this.shadows) { shadow.geometry.dispose(); }
        this.coins = [];
        this.shadows = [];
        this.coinData = [];
    }
...
function reloadVisibleChunks() {
    ...
    // --- Шаг 1: Определяем, какие чанки ДОЛЖНЫ БЫТЬ ЗАГРУЖЕНЫ (как и раньше) ---
    const chunksToKeep = new Set();
    for (let x = currentChunkX - settings.UNLOAD_DISTANCE; x <= currentChunkX + settings.UNLOAD_DISTANCE; x++) {
        for (let z = currentChunkZ - settings.UNLOAD_DISTANCE; z <= currentChunkZ + settings.UNLOAD_DISTANCE; z++) {
            chunksToKeep.add(`${x},${z}`);
        }
    }

    // --- Шаг 2: Проверяем существующие чанки и решаем их судьбу ---
    for (const [chunkId, chunk] of chunks.entries()) {
        if (!chunksToKeep.has(chunkId)) {
            // Этот чанк теперь за пределами даже самой дальней зоны выгрузки. Уничтожаем.
            chunk.destroy();
            chunks.delete(chunkId);
        }
    }

    // --- Шаг 3: Проходим по всем чанкам, которые должны существовать, и управляем их состоянием ---
    for (let x = currentChunkX - settings.UNLOAD_DISTANCE; x <= currentChunkX + settings.UNLOAD_DISTANCE; x++) {
        for (let z = currentChunkZ - settings.UNLOAD_DISTANCE; z <= currentChunkZ + settings.UNLOAD_DISTANCE; z++) {
            const distance = Math.hypot(x - currentChunkX, z - currentChunkZ);
            const chunkId = `${x},${z}`;
            ...
            if (distance <= settings.RENDER_DISTANCE) {
                ...
                if (!chunk) {
                    // Чанка нет, значит, он попал в видимую зону. Его нужно создать.
                    // Используем твою очередь загрузки, чтобы не создавать все сразу
                    if (!chunksToLoad.some(c => c.x === x && c.z === z)) {
                        chunksToLoad.push({ x, z });
                    }
                } else {
                    // Чанк уже есть. Убеждаемся, что он видим, и обновляем LOD.
                    ...
                    if (chunk.currentLODSegments !== requiredSegments) {
                        chunk.rebuildMesh(requiredSegments);
                    }
                }
            } else {
                // ЗОНА 2: БУФЕРНАЯ (между RENDER_DISTANCE и UNLOAD_DISTANCE)
                let chunk = chunks.get(chunkId);
                if (chunk && chunk.mesh) {
                    // Чанк существует, но он за пределами видимости. Просто скрываем его.
                    // НЕ УДАЛЯЕМ!
    ...
    // --- Шаг 4: Сортируем очередь загрузки (как и раньше) ---
    chunksToLoad.sort((a, b) => {
        const distA = Math.hypot(a.x - currentChunkX, a.z - currentChunkZ);
        const distB = Math.hypot(b.x - currentChunkX, b.z - currentChunkZ);
        return distA - distB;
    });
}
...
function updateChunks() {
    ...
    if (currentChunkX !== currentPlayerChunk.x || currentChunkZ !== currentPlayerChunk.z) {
        ...
        // Здесь остается ТОЛЬКО "ТИК". Он обновляет кулдауны, когда игрок движется.
        // Это действие подготавливает BonusManager к следующему вызову shouldSpawnBonus,
        // который произойдет, когда на горизонте будет создан новый чанк.
        if (bonusManager) {
            bonusManager.tickForPlayerTraversal(currentChunkX, currentChunkZ);
        }

        // Перезагружаем видимые чанки (LOD и т.д.)
...
function processChunkLoadQueue() {
    for (let i = 0; i < CHUNKS_PER_FRAME; i++) {
        ...
        if (!chunks.has(chunkId)) {
            ...
            // 1. Получаем текущие координаты чанка игрока.
            const playerChunkX = currentPlayerChunk.x;
            const playerChunkZ = currentPlayerChunk.z;

            // 2. Вычисляем расстояние в чанках от игрока до нового чанка.
            // Math.hypot - это то же самое, что и Math.sqrt(dx*dx + dz*dz)
            const distance = Math.hypot(chunkData.x - playerChunkX, chunkData.z - playerChunkZ);

            // 3. Передаем это расстояние в конструктор нового чанка.
            const newChunk = new Chunk(scene, chunkData.x, chunkData.z, distance);
...
setupSlider('GRAVITY');
setupSlider('RENDER_DISTANCE', false, 1, () => {
    updateFog();
    forceChunkUpdate();
});
...
function updatePhysics(delta, currentMaxSpeedCap, currentMaxPlayerSpeed) {
    ...
    if (!isPlayerOnGround) {
        ...
        const intersections = raycaster.intersectObjects(Array.from(chunks.values()).map(c => c.mesh), false);
    ...
    const collisionObjects = Array.from(chunks.values()).map(c => c.mesh);
    for (let i = 0; i < 5; i++) {
